...
|ULONG Lw_MsgQueue\_ SendEx ( LW_OBJECT_HANDLE ulId, PVOID pvMsgBuffer, size_t stMsgLen, ULONG ulOption );|
|---|
...
```
|\#include \<stdio.h\> \#include \<SylixOS.h\> int main (int argc, char \*\*argv) { int file = open ("/apps/time/a", O_CREAT \|O_RDWR, 0777); /\*创建文件 \*/ if(file == -1) { perror("fun open error\\n"); return PX_ERROR; } Lw_Backtrace_Show(file, 5); /\*显示调用栈信息\*/ close(file); return (0); }|
|---|
